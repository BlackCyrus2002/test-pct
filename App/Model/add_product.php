    ...
    if (!empty($_POST['nom_produit']) && !empty($_POST['desc_produit']) && !empty($_FILES['path_photo']['name'])) {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["path_photo"]["tmp_name"], $file)) {
                ...
                $stmt = $con->prepare("INSERT INTO produits (user_product, nom_produit, desc_produit, path_photo) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("ssss", $user_product, $nom_produit, $desc_produit, $photo_path);
                if ($stmt->execute()) {
